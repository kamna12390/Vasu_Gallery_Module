...
import com.vasu.image.video.pickrandom.galleryapp.activity.CropImageActivity
...
object CropImage {
    ...
    /** The key used to pass crop image bundle data to [CropImageActivity].  */
     var CROP_IMAGE_EXTRA_BUNDLE = "CROP_IMAGE_EXTRA_BUNDLE"
    ...
    fun todrawheart(bitmap: Bitmap): Bitmap? {
        val width = bitmap.width
        val height = bitmap.height
        val output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(output)
    ...
    fun todrawTriangle(bitmap: Bitmap): Bitmap? {
        val width = bitmap.width
        val height = bitmap.height
        val output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(output)
    ...
    fun todrawroundedrectangle(bitmap: Bitmap): Bitmap? {
        val width = bitmap.width
        val height = bitmap.height
        val output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(output)
    ...
    fun getHexagonShape(scaleBitmapImage: Bitmap): Bitmap? {
        val width = scaleBitmapImage.width
        val height = scaleBitmapImage.height
        val output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(output)
        val color = -0xbdbdbe
        val paint = Paint()
        ...
        path.lineTo(centerX - triangleHeight, (centerY + radius / 2).toFloat())
        path.lineTo(centerX - triangleHeight, (centerY - radius / 2).toFloat())
        path.lineTo(centerX.toFloat(), (centerY - radius).toFloat())
        path.lineTo(centerX + triangleHeight, (centerY - radius / 2).toFloat())
        path.lineTo(centerX + triangleHeight, (centerY + radius / 2).toFloat())
        path.moveTo(centerX.toFloat(), (centerY + radius).toFloat())
    ...
    fun torewfgefgv(bitmap: Bitmap): Bitmap? {
        val width = bitmap.width
        val height = bitmap.height
        val output = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(output)
        ...
        val x = width / 2f
        val y = height / 100f * 50.5f
        wallpath.moveTo(x, y) // used for first point
        ...
        paint.xfermode = PorterDuffXfermode(PorterDuff.Mode.SRC_IN)
        canvas.drawBitmap(bitmap, 0f, 0f, wallpaint)
 ...
 * question](http://stackoverflow.com/questions/32789027/android-m-camera-intent-permission-bug).
    ...
    )
    {
        ...
        private val mOptions: ImageOptions = ImageOptions()
        ...
        fun getIntent(context: Context, cls: Class<*>?): Intent {
            ...
            Log.d("TAG", "getIntent: check the data::$mSource")
            Log.d("TAG", "getIntent: check the data::$mOptions")
            Log.d("TAG", "getIntent: check the data::$bundle")
            return intent
        }
...
//            mOptions.validate()
//            activity.startActivityForResult(getIntent(activity).putExtra(Config.EXTRA_CONFIG, config), CROP_IMAGE_ACTIVITY_REQUEST_CODE)
//        }
    ...
    open class ActivityResult : CropImageView.CropResult, Parcelable {
        ...
        protected constructor(parcel: Parcel) : super(
            null,
            parcel.readParcelable<Parcelable>(Uri::class.java.classLoader) as Uri?,
            null,
            parcel.readParcelable<Parcelable>(Uri::class.java.classLoader) as Uri?,
            parcel.readSerializable() as Exception?,
            parcel.createFloatArray()!!,
            parcel.readParcelable<Parcelable>(Rect::class.java.classLoader) as Rect?,
            parcel.readParcelable<Parcelable>(Rect::class.java.classLoader) as Rect?,
            parcel.readInt(),
            parcel.readInt(),
        ) {
        ...
        companion object {
            ...
            val CREATOR: Parcelable.Creator<ActivityResult> = object : Parcelable.Creator<ActivityResult> {
                ...
                override fun createFromParcel(parcel: Parcel): ActivityResult {
                    ...
                    return ActivityResult(parcel) as ActivityResult
                }

                override fun newArray(size: Int): Array<ActivityResult?> {
                    return arrayOfNulls<ActivityResult>(size)
                }
            }
    } // endregion
}
